...
public class AdicionEmpleadoPlanilla {
    ...
    public int insertar() {
        ...
        try {
            ...
            ps.setInt(1, obtenerUltimoID()); // Usamos el ID de planilla pasado como parámetro
    ...
    /*public int insertar() {
        ...
        //System.out.println("el id desde Adicion empleado"+idp);
        try {
            PreparedStatement ps = Conexion.getConnection().prepareStatement("CALL sp_insert_tbl_detalle_planilla(?, ?, ?, ?, ?, ?, ?, ?)");
            ps.setLong(1, this.id_planilla); // Suponiendo que ya tienes el ID de planilla generado en otro lugar
            ps.setString(2, this.cedula_empleado);
            ps.setDouble(3, this.horas_trabajadas);
            ps.setDouble(4, this.salario_por_hora);
            ps.setDouble(5, SalarioBruto()); // Suponiendo que has implementado el método SalarioBruto()
            ps.setDouble(6, SeguroSocial()); // Suponiendo que has implementado el método SeguroSocial()
            ps.setDouble(7, SeguroEducativo()); // Suponiendo que has implementado el método SeguroEducativo()
            ps.setDouble(8, SalarioNeto());
